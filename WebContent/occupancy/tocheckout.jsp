...
<script type="text/javascript">  
	...
	function stay(){
		...
		var url = "<%=request.getContextPath()%>/occupancy/checkOut.do";
		$.post(url, {
			"in_no" : in_no,
			"days" : days,
			"foregift" : foregift,
 ...
 <div id="navbar">
	...
	<form action="<%=request.getContextPath() %>/room/query.do" class="form-inline" method="post">
		<div class="form-group" style="margin-bottom: 8px;min-width: 1000px;">
		          <!-- 续住天数&nbsp;<input type="text" name="day" id="day" class="form-control" style="width: 100px" onkeyup='this.value=this.value.replace(/\D/gi,"")' onpaste="return false;"> &nbsp;&nbsp; -->
			<button type="button" class="btn btn-primary" onclick="stay()">退房</button>
		</div>
	<div class="container">  	
			...
			<tbody>
				<c:forEach var="v" items="${list}">
					<tr>
						...
						<input type="hidden" id="in_time" value=${v.in_time } >
					</tr>
				</c:forEach>
			</tbody>
		</table>
