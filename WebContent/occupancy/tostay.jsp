...
<script type="text/javascript">  
	...
	function stay(){
		...
		if(Base.isInt(day)==false){
			layer.msg('天数格式错误，请重新输入！', {
				  offset: 't',
				  anim: 6
			});
		}else{
			var url = "<%=request.getContextPath()%>/occupancy/Stay.do";
			$.post(url, {
				"in_no" : in_no,
				"day" : day,
				"days" : days,
 ...
 <div id="navbar">
	...
	<form action="<%=request.getContextPath() %>/room/query.do" class="form-inline" method="post">
		<div class="form-group" style="margin-bottom: 8px;min-width: 1000px;">
		          <b>续住天数:</b>&nbsp;<input type="text" name="day" id="day" class="form-control" style="width: 100px" onkeyup='this.value=this.value.replace(/\D/gi,"")' onpaste="return false;"> &nbsp;&nbsp;
			<button type="button" class="btn btn-primary" onclick="stay()">确认续住</button>
		</div>
	<div class="container">  	
